...
describe('loadUserSshConfig', () => {
  ...
  it('returns no hosts and no skips when the user config does not exist', () => {
    makeHome()
    // An absent ~/.ssh/config is a real answer, so there is nothing unread to report.
    expect(loadUserSshConfig()).toEqual({ hosts: [], skippedIncludes: [] })
  })
})
