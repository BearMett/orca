...
import {
  ...
  restoreCanonicalMarkdownSourceEof
...
export function commitRichMarkdownSerialization(
  ...
  try {
    ...
    reconciled = restoreCanonicalMarkdownSourceEof(edited, refs.originalSourceRef.current)
  }

  refs.originalSourceRef.current = reconciled
  // Why: reconciled ≡ edited semantically, so its canonical form is `edited`
  // (POSIX trailing newline is not part of getMarkdown).
